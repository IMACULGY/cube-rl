...
class MCTS:
    def __init__(self, cube_env, state, alg, net, exploration_c=100, virt_loss_nu=100.0):
        # initialize cube and model info
        self.cube_env = cube_env
        self.root_state = tuple(encode(state))
        self.alg = alg
        self.net = net
        ...
        # keep track of how to get to each state for each time we need to revisit
        self.states = {self.root_state:self.alg}

        # Tree state
        shape = (cube_env.actionsize, )
    ...
    # set the env to the given state
    def visit(self, s):
        # reset to solved state
        self.cube_env.reset(0)
        # perform algorithm
        self.cube_env.pycube(self.states[s])

    # update state
    def updatestates(self, s, childrens):
        actions = self.cube_env.action_list
        # add the new move to the old algorithm for state
        for i in range(self.cube_env.actionsize):
            if not childrens[i] in self.states:
                self.states[childrens[i]] = f"{self.states[s]} {actions[i]}"

    def explore(self, s):
        self.visit(s)
        child_states, _, child_goal = self.cube_env.explore()
        # encode the states
        child_states = list(map(lambda x:tuple(encode(x)), child_states))
        # update
        self.updatestates(s, child_states)
        return child_states, child_goal

    def render(self, s):
        self.visit(s)
        self.cube_env.render()
    ...
    def search(self):
        ...
        child_states, child_goal = self.explore(s)
    ...
    def _search_leaf(self):
        ...
        while True:
            ...
            N_sqrt = np.sqrt(np.sum(act_counts))
            if N_sqrt < 1e-6:
                act = random.randrange(self.cube_env.actionsize)
            else:
                u = self.exploration_c * N_sqrt / (act_counts + 1)
                u *= self.prob_actions[s]
                q = self.val_maxes[s] - self.virt_loss[s]
    ...
    def evaluate_states(self, states):
        ...
        enc_states_t = torch.tensor(states).to(self.device)
    ...
    def dump_solution(self, solution):
        ...
        for aidx in solution:
            a = self.cube_env.action_list[aidx]
            print(a, aidx)
            s, _, _ = self.cube_env.step(a)
            self.cube_env.render()
    ...
    def find_solution(self):
        ...
        while queue:
            ...
            seen.add(s)
            c_states, c_goals = self.explore(s)
...
# searcher = MCTS(env, state, alg, net)
# #searcher.dump_root()
# result = None
...
# searcher.dump_root()
# searcher.dump_solution(result)
# result = [env.action_list[i] for i in result]
